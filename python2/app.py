...
def check(email):
    url = " https://x9xunxuut6.execute-api.us-west-2.amazonaws.com/plasma1/getdata1?email="+email
...
def register():
    ...
    if('errorType' in check(x[1])):
        url = " https://x9xunxuut6.execute-api.us-west-2.amazonaws.com/plasma1/registration1?"+params
...
@app.route('/stats')
def stats():
    url = " https://x9xunxuut6.execute-api.us-west-2.amazonaws.com/plasma1/getbloodgroupsdata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    #print(bloodgrp)
    url = " https://x9xunxuut6.execute-api.us-west-2.amazonaws.com/plasma1/requestonbloodgroup?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    print(a)
    phone=[]
    for i in a:
        url="https://www.fast2sms.com/dev/bulk?authorization=ZRyUXglOuI15Vd4mSiMpxW0JBvjachGHKAfoQ8kTCEDw9be3rzsSmJ1drDC6bnfGNR0TvPU4XiaZhxOM&sender_id=FSTSMS&message=Need Plasma of your blood group&language=english&route=p&numbers="+str(i['phone'])
